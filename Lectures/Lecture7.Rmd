...
* SVD (singular value decomposition) of a matrix is used to find horizontal & vertical vectors (singular vectors) & normalizing values (singular values)
...
* Matrix X here is said to be rank-1 b/c _u_ and _v_ both have one column  
...
* Decomposition of X is not unique, there are several candidates for vectors _u_ & _v_ (3 different ways to get area of rectangles as above)
  ...
  * Keep track of one extra number (#used to multiply each of the products & represents "overall scale" of X)
    * This is the #in the white box at the upper left corner of matrix 3 above (2348.3) = **singular value s1**
...
sum(u^2)
sum(v^2)
s1*u %*% t(v) #note find out what %*% does later
X-s1*u %*% t(v)
```
...
* In this case, can see the 2nd & 3rd singular values are 0 (not looking at decimals)
...
* 3rd & 4th values are small, don't improve approximation -> conclude Xtwo is rank-2
...
* Q) Compute 1st PC for turtles data by multiplying by 1st singular value of usv$d[1] by usv$u[,1]. What is another way of computing it?
* A) 
```{r}
...
Matrix multiplications can show that XV & US are the same
  * Remember V is orthogonal (so V<sup>t</sup>V = I & XV=USV<sup>t</sup>V=USI)
