...
#### 7.1
* Can decompose a matrix X with n row x p column rank 1 
...
* 7.1a) If matrix X has no rows and no columns which are all zeros, then is this decomposition unique?
  * No, several candidates for vectors u & v
...
* 7.1b) Generate a rank-one matrix: Start by taking a vector of length 15 with values from 2 to 30 in increments of 2, and a vector of length 4 with values 3,6,9,12, take their ‘product’
  * Why do we have to take t(v)? -> from SVD formula SVD=USV^t where U is made from columns of AA^t, and V is made of corresponding rows by A^tA (in other words, column V^t matches columns of U via eigenvalue calcs (? - need to check))
...
X1
X1.1 <- prcomp(X1)
svd(X1)$d #shows one relevant singular value >0 so rank 1

# check eigenvalues/singular value
fviz_screeplot(X1.1) + ggtitle("")
```

* 7.1c) Now we add some noise in the form a matrix we call Materr so we have an “approximately rank-one” matrix.
...
#Plot PC1 & 2
Xcc <- tibble(PC1x = Xc$u[,1] * Xc$d[1], PC2x = Xc$u[,2] * Xc$d[2]) #melt+ggpairs?(level plot, take each value of matrix and assign a number), geomraster
# ggpairs() = correlation 1 b/c one column
ggplot(Xcc, aes(x=PC1x, y=PC2x)) + geom_point()
```

* 7.1d) Redo the same analyses with a rank 2 matrix.

```{r}
#using above athletes data
fviz_eig(pca.ath, geom="bar", bar_width=0.3) + ggtitle("") #visualize rank 2
pca.ath_1 <- tibble(PC1=pca.ath$li[,1], PC2=pca.ath$li[,2])
ggplot(pca.ath_1, aes(x=PC1, y=PC2)) + geom_point()
```
...
sim2d = data.frame(MASS::mvrnorm(50, mu = c(mu1,mu2), Sigma = sigma))
sim2d_1 <- prcomp(sim2d, scale=T)

fviz_screeplot(sim2d_1) + ggtitle("") #or svd(scale(sim2d))$d
```

* 7.2B) Perform a PCA and show the rotated principal component axes.

```{r}
sim2d_2 <- tibble(PC1=sim2d_1$x[,1], PC2=sim2d_1$x[,2])
ggplot(sim2d_2, aes(x=PC1, y=PC2)) + geom_point()
```
...
* Where is figure 7.35??
  * But based on ?coord_fixed() 
...
```{r}
pcaUW = dudi.pca(as.data.frame(t(Biobase::exprs(xwt))),
  center = TRUE, scale = TRUE, nf = 2, scannf = FALSE)
fviz_pca_var(pcaUW, col.circle="black") + ggtitle("")
```
...
```{r}
fviz_pca_biplot(pcaUW, label="var", habillage = xwt$sampleGroup)  + 
  coord_fixed() + ggtitle("")
```
