...
<img src="https://ars.els-cdn.com/content/image/1-s2.0-S007668791830288X-f01-02-9780128159934.jpg?_"  width="350" height="500">
...
> We bigin with gene count data from an experiment on effect of RNAi knockdown of Pasilla, the Drosophila melanogaster ortholog of mammalian NOVA1 and NOVA2, on the transcriptome.
...
* Current RNA-seq data -> short fragments of full-length isoforms = hard to assemble full length isoform structures/abundances
  * Procedures have modest aim of making local statements (inclusion/exclusion of individual exons) = more robust

<button data-toggle="collapse" data-target="#e1">Abdul's Notes</button>
...
<button data-toggle="collapse" data-target="#c1">condensed notes</button>
<div id="c1" class="collapse">
[link](https://github.com/hbctraining/DGE_workshop/blob/master/lessons/02_DGE_count_normalization.md)

* n=total library size (total #fragemnts)
* <u>normalization (AKA size/proportional factor)</u>
  * 1: (A-mean) geometric mean of row (gene1 (row) per samples (column)
  * 2: (B-ratio) divide each fragment count by gene (row) geometric mean - ASSUMES GENES NOT DEFERENTIALLY EXPRESSED (therefore, similar ratio outcome) - accounts for difference in sequencing depth & RNA abundance per sample
  * 3: (C-size factor) calculate size factor -> this is the median value for the B-ratio per sample (column)
    * size factor of 1 = large variation between samples (outliers presence)
  * 4: (D-scale) Use size factor to normalize gene counts (rows) by dividing

* AKA normalization controls for seq depth difference in gene abundance & RNA difference in gene abundance
* Comparing groups with the same genes -> don't need to account for length difference


<img src="Dispersion.png">

> Dispersion -> gamma-Poisson b/c lambda = variance, also lambda also calculated from np (n=total read count * proportion of that gene) 

https://www.annualreviews.org/doi/full/10.1146/annurev-biodatasci-072018-021255#_i44
</div> 
...
* Ex) Dataset w/ 5 genes + 2 samples
  ...
  * If we estimate s so that ratios -> slope of red line
    ...
    * This estimate is more parsimonious & perferred
      * Slope via robust regression via DESEQ2
...
stopifnot(!any(is.na(mt))) #ordering putting untreated first for easier downstream analysis
...
```{r}
DESeq2::plotMA(pasilla, ylim=c(-2,2))
```
...
pasilla1.1 = DESeqDataSetFromMatrix(
  countData = counts,
  colData   = pasillaSampleAnno[mt, ],
  design    = ~ condition)
class(pasilla1.1)
p= estimateSizeFactors(pasilla1.1, controlGenes= (1:130))
    ...
    * **Methods used on least sum of squares have low breakdown pt**
      ...
      * If median is higher vs breakdown pt -> changing single# has no effect
        ...
        * To change median by high amount -> need to change half of the obs
          * **Median is robust -> breakdown point is 1/2**
          * median of set of #'s y1,y2,... minimizes the sum
          
          \begin{equation}\sum_j|y_j-\beta_0|\end{equation}

* To get a high degree of robustness vs outliers, there choices other than sum of squares(eqn 8.6) used for minimization
...
* All these approaches to outlier robustness add a subjective component & **rely on sufficient replication**
...
<button data-toggle="collapse" data-target="#c2">condensed notes</button>
<div id="c2" class="collapse">
* R = residuals (outliers), want to minimize between replicates
* Methods
  * 1: Minimize sum of squared residuals (least sum of squares fitting)
    * problem = sensitive to outliers because "linear model"
    * Low breakdown point (breakdown point is 1/2 of median) so need to increase #obs to increase breakdown point
  * 2: Least absolute deviation (median generalization)
    * problem = harder to minimize residuals vs above squared residuals
  * 3: M-estimate
    * Requires knowledge of data from literature for s value(outlier value cutoff)
  * 4: LTS/LQS
    * Generalizes least sum of squares (eqn 1)
    * Uses quantiles (LQS -> sum of least quantiles squares)
    * LTS is sum of squared residuals but only for the smallest residuals (0<= theta <=1)
  * 5: General weighted regression
    * Gives weights to observations to find outliers (DESeq uses Cook's algorithm for each gene difference B, if cook's = 0 -> large distance AKA outlier discarded)
</div>
    
* Task: Search for packages implementing the above eqns for robust regression methods <mark style='background-color:red'>skipped</mark>
...
* For linear models, values of y should cover the whole real axis
  * If transforming the expression on the right w/ a sigmoid function (ex) f(y) = 1/(1+e<sup>-y</sup>) -> the range of the function is bound between 0 & 1 and more useful for the disease risk model above
...
* Generalizing the maximization criterion (eqn 8.6)
  * This criteria can be derived via specific probabilistic model (Maximium likelihood principle - seen in CH2)
...
* Core concepts 
  * Left hand side of eqn 8.13 (product of probabilities of residuals) = function of the model parameters β1,β2,... & data y1, y2,... -> this is f(β,y)
    * Think of the model parameters β as fixed
      * Then the collapsed function f(y) indicates the probability of the data
    * Think of the data given (where β not fixed)
      ...
      * We use this to optimize criteria 8.6 (which changes criteria 8.13 also)
        * β's obtained are called maximum-likelihood estimates 
...
* Step-by-step
  ...
  * Eqn 8.15 states that mean made from sample-specific size factor (sj & qij)
    * This is proportional to the true expected concentration of fragments for gene i & sample j
...
* The above figure shows p-values in 2-factor analysis similar to 1-factor analysis
  * 2-factor analysis generally smaller
...
* A) There are methods attempting to identify blocking factors via unsupervised learning (read Leek and Storey (2007; Stegle et al. 2010))
...
* Priors in this case is from distributions of maximum-likelihood estimates (MLEs) across all genes
...
<img src="http://web.stanford.edu/class/bios221/book/figure/chap7-rnaseq-posterior-1.png"  width="250" height="250"> 
<img src="http://web.stanford.edu/class/bios221/book/figure/chap7-rnaseq-posterior-2.png"  width="250" height="250">

> Figure 8.10: Shrinkage estimation of logarithmic fold change estimates by use of an empirical prior in <strong><a href="https://bioconductor.org/packages/DESeq2/">DESeq2</a></strong>. Two genes with similar mean count and MLE logarithmic fold change are highlighted in green and blue. The normalized counts for these genes (left panel) reveal low dispersion for the gene in blue and high dispersion for the gene in green. In the right panel, the density plots are shown of the normalized likelihoods (solid lines) and of the posteriors (dashed lines) for the green and blue gene. In addition, the solid black line shows the prior estimated from the MLEs of all genes. Due to the higher dispersion of the green gene, its likelihood is wider and less sharp (indicating less information - wider variance/lower mean), and the prior has more influence on its posterior than in the case of the blue gene.
...
<button data-toggle="collapse" data-target="#f1">Abdul's Notes</button>
...
* w/ many degrees of freedom (more samples than #of parameters to be estimated), may not want to remove entire genes from analysis b/c their data have a single count outlier
  * Alternative strategy is to replace #outlier counts w/ trimmed mean over all samples & adjust by size factor for the sample
...
* How to add requirement for detecting effects w/ strong size (but small) vs statistically significant ones?
...
myMA = function(h, v, theta = 0.5) {
  DESeq2::plotMA(pasilla, lfcThreshold = theta, altHypothesis = h,
         ylim = c(-2.5, 2.5))
...
<button data-toggle="collapse" data-target="#k1">condensed notes</button>
<div id="k1" class="collapse">
* Shrinkage is based upon Bayesian statistics
  * Prior knowledge of expected provided = decrease false +ve rates
  * increase #sames = decrease shrinkage
  * genes w/ low dispersion is shrunken towards curve (dispersion vs mean normalized counts) = more accurate
  
* Wald's hypothesis test
  * set up H0 for each gene (= no differential gene expression accross 2 groups (LFC=0))
  * Use statistical test to determine if H0 true
    * Wald's is used to compare against 2 groups (small p-value=rej H0 -> evidence for differential expression (DE))
...
#model.matrix() forms a design matrix
  #design <- model.matrix(~ condition + type, pasillaEdge$samples) #account for both condition and type variables
design <- model.matrix(~ condition, pasillaEdge$samples) #account only for condition variable like in 8.5
pasillaEdge1 <- calcNormFactors(pasillaEdge, method="RLE") # estimate size factor
pasillaEdge1 <- estimateDisp(pasillaEdge1, design) #estimate dispersion - need to specify design matrix
fit <- glmFit(pasillaEdge1, design=design) # Fit to a linearized model, note dispersion is set at NULL default
lrt <- glmLRT(fit, coef=ncol(design)) #choose coefficient to test for from design table
ttL <- edgeR::topTags(lrt, n=nrow(lrt), sort.by="none") #extract summary table
```

```{r}
#Compare w/ DESeq2 output
  # edgeR normalization = Trimmed Mean of M-values normalization (TMM) default
  # DESeq2 normalization = Relative Log Expression” normalization (RLE) default
    # median based normalization
  # normalization factors edgeR =/= size factors DESeq2
pasilla$sizeFactor ; pasillaEdge1$samples$norm.factors #size factor est.
```

```{r}
#Compare edgeR results vs DESeq2 results
table(DESeq2=results(pasilla)$padj < 0.1, edgeR=ttL$table$FDR <0.1)
```

```{r}
tt <- topTags(lrt, n=nrow(pasillaEdge1))
tt10 <- topTags(lrt) # just the top 10 by default
tt.all <- topTags(lrt, n=nrow(pasillaEdge1), sort.by="none")
```

```{r}
# Compare the two by ranks
...
<button data-toggle="collapse" data-target="#g1">Abdul's Solution</button>
<div id="g1" class="collapse">


```{r}
y <- calcNormFactors(pasillaEdge1, method="RLE")
