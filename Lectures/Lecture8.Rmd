...
* In addition to siRNA knockdown of pasilla gene, want to ttest effect of a drug.
  ...
  * Use the notaion 
    ...
    * Coefficient B0 = base lvel of measurement in -ve control (also called the intercept)
    * Design factors x1 & x2 = binary indicator variables
      ...
      * In this exp only siRNA used, no drugs (x1=1 & x2=0)
        * 3rd and 4th term of this eqn simplified to 0
          * so final eqn: y=B0+B1
            * B1 will represent the difference between treatment + control
...
> sometimes written w/ additional term x0 multiplied w/ B0 & x0=1 always.  
This makes subsequent notation easier to keep track of b/c intercept easier to handle w/ B1 & B2
...
* If measurements are on log scale then following eqn is on log fold change to treatment w/ siRNA
  * B2 is log fold change due to treatment w/ drug (x2) (as with previous but w/ log)
...
* B12 is interpreted as "difference between observed outcome (y) & expected outcome from individual treatments (via adding to baseline the effect of siRNA alone (B1) + drug alone (B2)"
  * B12 = interaction effect of siRNA and drug
...
* ex) Target drug of siRNA equivalent -> same side effect on cells then biologically expect
  * B1=B2
  * Also expect that combination of the drug + siRNA lead to no further effects (additive), then B12= -B1
  
* ex) Target drug of siRNA are parallel pathways (ie) buffer each other)
  * Expect B1 & B2 to be small
  * Also expect that the combination of drug + siRNA lead to synergistic effect, and B12 is large
...
* Don't always care about interactions
  ...
  * In this case -> combinatorial treatment may not be present in experimental design
    * Model to use for analysis = eqn 8.1 above but remove end right term
...
* If combinatorial treatment is not performed, then design matrix reduced to 3 rows of the above matrix
...
* If data (y's) were precise, then can use a linear system of eqns
  * one eqn for each of the 4 exp. conditions (x's /rows) -> can solve for B's
* Usually want to analyze real data affected w/ noise
  * **Need replicates to estimate lvls of noise + assess uncertainty of estimated B's**
...
* Added new term (Ej) & index (j) (Xj1, etc) to eqn 8.1
  * index j = explicitly counts over individual replicates in exp.
    * ex) if replicate 3 times for each of the 4 conditions -> j counts from 1 to 12 (AKA now design matrix has 12 roes & x<sub>jk</sub> is value of the matrix in _j_th row & _k_th column)
    ...
    * Needs one additional modeling component
      * the 12 modeling component in eqn 8.5 needs further info 
        ...
        * Solution: **Ej needs to be small**
          * One method = minimize sum of squared residueals
...
> Since B0 = the intercept, then x<sub>j0</sub>=1 for all j

* This requirement is satisfied b/c 
  * B's represent average effects of each experimental factors
...
* Q) Alternative to writting eqn 8.5 is the following:
  * How can this be mapped to eqn 8.5 (what's with the interation term xj1 xj2 B12)?
* A) Notation -> sum extends over k=0,...,3
  * Terms for k=0,1,2 are known
  * Write B3 instead of B12 & xj3 = (xj1)(xj2)
...
* Think about eqn 8.5 this way
  * **Analysis of variance (ANOVA)**
    * eqn 8.5 decomposes the variabillity of (y) observed during experiments into elementary components
      * B0 = Baseline value -> variability caused via effect of 1st variable (B1)
        * B1 variability caused by effect of 2nd variable B2
        * B2 variability caused by effect of interation B12
        * B12 variability unaccounted for
          * unaccounted variability = noise/systematic variability

#### Rombustness
* **Sum of eqn 8.6 is sensitive to data outliers**
  * One measurement (yj) w/ outlier value changes B estimate to be far from values measured by replicates
...
* Eqn 8.11 is a complex way of weighing down outliers
  * Assumes there is a way to decide the weight (wj) for each observation (which should weigh down outliers)
    * In "Further statistical concepts" of this CH -> DEseq package approach
      * Leverage of each data point on estimated B's assessed via Cook's distance
