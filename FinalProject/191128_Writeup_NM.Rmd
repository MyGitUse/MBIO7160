...
<a href="#2">[2]</a> Background of Dataset      
...
&emsp; Principal component analysis (PCA) is commonly used in ecological studies, such as bacterial microbiota & fungal mycobiota analyses, with many papers not providing a clear interpretation of the results. How does PCA reduce data dimensionality and what is the interpretation, also is PCA an appropriate method for "-biota" analyses? I will investigate the mathematics behind PCA and determine how dimensionality is reduced. Furthermore, I will examine if PCA is appropriate for community driven beta-diversity analysis, which infers ecological structural differences between individuals.  
&emsp; This is important because inferring beta-diversity differences between individuals can give us an insight on core community diversity between a normobiosis patient versus a dysbiosis patient. This meaning, the organisms residing in a healthy individual may vastly differ when compared to an individual suffering from a particular disease. If this statement holds true, then PCA, or other dimensionality reduction techniques, may give us insight on whether differences in community composition is associative to a particular disease (such as immune mediated inflammatory diseases).  
&emsp; My investigation into PCA has led me to unsupervised dimensionality reduction methods, called ordination. Collectively, these methods attempt to reduce dimensions of a multivariate dataset while retaining ordered grouping relationships between variables. The goal of ordination methods is to provide visualization of grouped relationships in a reduced multidimensional space. I will address three ordination techniques and attempt to determine which works better for community-based beta-diversity analysis.

* **
<p id="2"><b><font size="5">Background of Dataset</b></font><a href="#0"><sup>Return</sup></a></p>
* **

&emsp; The dataset I have chosen for this study comes from the authors [Zuo et al. (2019)](https://www.nature.com/articles/s41467-018-06103-6/). This is a study conducted on patients suffering from recurrent _Clostridium difficile_ infection (CDI), who have underwent fecal microbiota transplant (FMT) during the course of the study by using the fecal organism community from a matched donor control patient. _C.difficile_ is a bacterial organism that can infect the human intestine, leading to inflammation and causing diarrhea, fevers, bloody stool, as well as abdominal pains.  
&emsp; Current treatment for CDI infection involves antibiotic intervention, but _C.difficile_ is becoming adaptively resistant as well as inherently resistant due to sporulation inactivity. As such, FMT treatment is becoming an increasingly effective method in treating recurrent CDI. The idea behind FMT is, a matched healthy donors stool sample will contain a normobiosis community of bacterial organisms. Once the donor stool is transplanted into a CDI patient, this normobiosis community of organisms is suspected to outcompete _C.difficile_ colonization of the gut. This should result in decreased CDI and associated symptoms because the organisms from the donor community becomes inhabitants the CDI patients gut.  
&emsp; The authors of this particular study is interested in the fungal community, the mycobiome, of the donors fecal sample as compared to their FMT matched CDI patient. Of note, the authors sequenced the fungal internal transcribed spacer (ITS2) region of 16 donor patient stools and their matched CDI recipients. They investigated the fungal communities through metataxonomic analysis, while collecting longitudinal samples from CDI patients during and post-FMT (67 samples in total). Out of the 16 FMTs, 9 patients exhibited no CDI symptoms (CDI1 to CDI9) and their fecal communities became similar to that of the donors; 7 patients were partially/not cured, and after a certain period of time, they reverted to recurrent CDI.  
&emsp; This study implies that the gut fungal community of an individual may play a role in preventing recurrent CDI. As such, I will apply ordination methodologies to see if the donors fungal community is similar to the CDI recipient community, especially in the case of the 9 cured patients. 
...
&emsp;  As previously stated, ordination based techniques are useful for ordering grouped relationships and in particular for ecological community analyses. Ordination works by providing a visualization of multivariate data due to a reduction of dimensions while maintaining the most relevant information contributing to the data (McCarthy, n.d.; OM, n.d.). This allows us to visualize data that would otherwise be humanly impossible to view in higher dimensions. In indirect ordination analysis, a counts table containing "species" abundance for a certain ecological community is used for analysis. Indirect ordination is able to infer lurking relationships by using the redundancy within a dataset. This is because multiple "species" within the count table will share a similar distribution of abundances which forms redundancy, an advantage used by ordination to order related groups.  
&emsp; An Eigenanalysis based technique is the principal component analysis (PCA), which uses linear matrix algebra to calculate Eigenvalues and their associated Eigenvectors (Lever, 2017; McCarthy, n.d.; Shlens, 2014; Smith, 2002; Wold, 1987). Eigen calculations are performed by Eigendecomposition or singular value decomposition (SVD), both of which yield similar results (OM, n.d.; SV, n.d.). In Eigendecompostion, the data is first scaled by standardization (dividing the observations by the column standard deviation) and/or centered (subtracting the observations by the column mean). Standardization is performed if the compared variables (the columns) are in different units of measurements, which does not have to be performed on ecological counts abundance tables because the measurements are unitless. While centering is performed to bring the centroid of the group of points to the (0,0) coordinate, without altering the ordering. Centering in particular is not essential to the analysis, as it does not effect the overall ordering.  
&emsp; Eigenanalysis requires a squared/symmetrical matrix to perform linear operations, and this is established through correlation or covariance calculation of the initial matrix. In the case of correlation calculations, this is again used if the initial variables contain different units. Again, ecological counts tables are unitless, so a covariance calculation is appropriate in generating a symmetrical covariance matrix for linear application. Both correlation and covariance calculations depend on comparing between two variables ("species") of a particular sample, which helps distinguish relationships within a community.  
&emsp; Without going into too much mathematics, the next step is to calculate Eigenvalues by finding the determinant of the squared covariance matrix. The number of Eigenvalues should be equivalent to the lowest number of variables in the dataset (either number of samples, or number of "species"), and eigenvalues indicate variance which is used to project a lower dimensionality (AKA principal components (PCs)). Using the calculated Eigenvalues, the corresponding Eigenvectors are next calculated and normalized based upon the associated Eigenvalue. Eigenvectors correspond the the weights/loadings used to plot the new coordinates of datapoints in a reduced space.  
&emsp; Since multiple PCs are generated, Eigenvalues (PCs) are visualized as a barchart (screeplot) based upon the proportion of the total variation accounted by each PC. The most informative PCs will account for the most variance and should be used to visualize the transformed data in reduced space. This is dimensionality reduction while preventing loss of too much information because PCs accounting for low variation in data is assumed to be not very informative in finding ordered relationships. It should be noted that each PC is orthogonal to eachother, so this indicates an initial assumption of no relation between axes.    
&emsp; There are certain limitations in performing PCA, one of which is because PCA calculation using a covariance/correlation matrix is a Euclidean measure of distance. This means that it assumes the data is linearly connected, which is why PCA is a linear model of ordination. This is especially bad in the case of ecological community analysis, where data is very sparse with a high number of 0 counts. The result of this limitation is seen in my analysis, where a "horseshoe"-like shape is visualized and is not very informative (<a href="#fig2">Figure 2</a>). This "horseshoe"-like shape is said to appear when there is a high amount of beta-diversity between samples (OM, n.d.).  
&emsp; Other ordination techniques involve using a distance matrix instead of covariance/correlation matrix for finding ordered relationships. Metric multidimensional scaling (MDS), also known as principal coordinate analysis (PCoA), is one of such techniques (McCarthy, n.d; MS, n.d.; OM, n.d). MDS is exactly the same as PCA, in the sense that it performs Eigenanalysis. In fact, if you used a Euclidean measure for a distance matrix and input this matrix to calculate MDS, the output would be the exact same as PCA (Figures <a href="#fig2">2</a> & <a href="#fig7">7</a>). MDS is an improvement compared to PCA because it allows you to provide distance measures other than Euclidean, this escaping the linear limitation of Euclidean distance; however, MDS attempts to maximize linear distances between points. So the major limitation to MDS is finding a distance measure which best fits the data.  
&emsp; Similarly, nonmetric multidimensional scaling (NMDS) also intakes a distance matrix for calculation (McCarthy, n.d; MS, n.d.; OM, n.d). This technique differs from PCA and MDS, in the sense that the order of the axes is arbitrary so that no one axis is more important than the other. Although I do not have a complete grasp of this method, I understand that the main goal is to separate similar points from dissimilar points. Also, NMDS does not by maximize linear distances but maximizes ranking orders. This process is said to be iterative, in which you first choose the number of dimensions to transform data observations. The iteration process causes convergence at some point, or stops when a low stress value is calculated (a stress value is a goodness-of-fit measure). Due to time limitation on this project, I plan on looking into the NMDS algorithm further in the future.
...
&emsp; Using the edited `pca1()` function, I set the arguments inputting the OTUanalysis dataframe, and groupingstable for grouping information, as well as using centering scaling by subtracting column means. I then obtained transformed coordinates for multiple principal components (PCs), which is based upon the number of columns (OTU) in the OTUanalysis dataframe. In the first step of PCA, I visualized the PCs using the `screeplot()` function, which indicated PC1 to PC3 accounting for most of the sample variance (<a href="#fig1">Figure 1</a>). I then plotted a `biplot()` to see how PC2 against PC1 would look like in 2-dimension, it has a v-like/parabolic shape which is a limitation of PCA (<a href="#fig2">Figure 2</a>).  
...
&emsp; I double checked these results using step-by-step Eigendecomposition calculations, as well as an alternative PCA analysis tool "factoextra" mentioned in the textbook and the results correspond well (Figures <a href="#fig4">4</a> & <a href="#fig5">5</a>). The codes used for this section is found below in the <a href="#PCA">"PCA Analysis"</a> codes section. 


<u>**MDS and NMDS Analysis**</u>  
&emsp; MDS and NMDS analysis both intake a distance matrix to perform matrix transformations. Starting with MDS, I used the "vegan" package and the function `vegdist()` to perform a distance measure using the Jaccard distance method. Then I used "stats" package which allows for MDS based Eigen calculations on a distance matrix. By using the function `cmdscale()`, where I selected the argument "k=2", the output is for components MDS1 and MDS2 which are equivalent to PCs in PCA; however, the similarity of CDI cured samples, and their matching controls, are grouped more distinctively when compared to PCA (Figures <a href="#fig6">6</a> & <a href="#fig2">2</a>)  
&emsp; I also analyzed MDS using the Euclidean distance measure, and the output is identical to the above PCA analysis (<a href="#fig7">Figure 7</a>). This is expected because PCA is known as a Euclidean method of ordination analysis. Alternatively, Bray–Curtis dissimilarity could have been used to calculate the distance matrix, which would result similar to the Jaccard distance calculation (Figures <a href="#fig6">6</a> & <a href="#fig8">8</a>). I chose the Jaccard measure arbitrarily, which is a limitation of this study because finding the correct distance measure for the data requires further investigation. The codes used for MDS analysis is found at the <a href="#MDS">"MDS Analysis"</a> section in codes.  
&emsp; For NMDS analysis, I again used the "vegan" package with the function `metaMDS()` which iteratively tries to calculate the lowest stress value (a goodness of fit measure). The number of iterations, as well as stress value, will differ per run because `metaMDS()` starts calculations at different points. For this analysis I kept the distance measure as Jaccard to compare with the MDS method. I also kept the same argument, "k=2", to calculate for components NMDS1 and NMDS2. The output graph more distinctly orders CDI cured patients with their matched controls, as compared to MDS (<a href="#fig9">Figure 9</a>).   
&emsp; To check between distance measures, I applied the Bray–Curtis distance measure in NMDS and it resulted in similar plot to the Jaccard measure NMDS plot (<a href="#fig10">Figure 10</a>). Codes used in NMDS analysis is found <a href="#NMDS>">below</a>. 
...
[1] Hildebrand, F., Tadeo, R., Voigt, A., Bork, P., & Raes, J. (2014). LotuS: an efficient and user-friendly OTU processing pipeline. Microbiome, 2(1), 30. https://doi.org/10.1186/2049-2618-2-30

[2] Lever, J., Krzywinski, M., & Altman, N. (2017). Principal component analysis. Nature Methods, 14(7), 641–642. https://doi.org/10.1038/nmeth.4346

[3] McCarthy, B. (n.d.). Ordination & PCA. [pdf] Available at: https://www.ohio.edu/plantbio/staff/mccarthy/multivariate/Ordination&PCA2012.pdf [Accessed 3 Dec. 2019].

[4] McCarthy, B. (n.d.). PCO and NMDS. [pdf] Available at: https://www.ohio.edu/plantbio/staff/mccarthy/multivariate/PCO&NMDS.pdf [Accessed 3 Dec. 2019].

[5] (MS) Multidimensional Scaling. (n.d.). [pdf] NCSS. Available at: https://ncss-wpengine.netdna-ssl.com/wp-content/themes/ncss/pdf/Procedures/NCSS/Multidimensional_Scaling

[6] (OM) Ordination Methods - an Overview. (n.d.). Retrieved December 4, 2019, from http://ordination.okstate.edu/overview.htm

[7] Shlens, J. (2014). A Tutorial on Principal Component Analysis. [pdf] Mountain View, CA. Available at: https://arxiv.org/pdf/1404.1100.pdf [Accessed 3 Dec. 2019].

[8] (SV) Singular Value Decomposition (SVD) tutorial. (n.d.). Retrieved December 3, 2019, from https://web.mit.edu/be.400/www/SVD/Singular_Value_Decomposition.htm

[9] Smith, L. (2002). A tutorial on Principal Components Analysis. [pdf] Available at:  http://www.iro.umontreal.ca/~pift6080/H09/documents/papers/pca_tutorial.pdf [Accessed 3 Dec. 2019].

[10] Wold, S., Esbensen, K., & Geladi, P. (1987). Principal component analysis. Chemometrics and Intelligent Laboratory Systems, 2(1–3), 37–52. https://doi.org/10.1016/0169-7439(87)80084-9

[11] Zuo, T., Wong, S. H., Cheung, C. P., Lam, K., Lui, R., Cheung, K., … Ng, S. C. (2018). Gut fungal dysbiosis correlates with reduced efficacy of fecal microbiota transplantation in Clostridium difficile infection. Nature Communications, 9(1),  3663. https://doi.org/10.1038/s41467-018-06103-6
