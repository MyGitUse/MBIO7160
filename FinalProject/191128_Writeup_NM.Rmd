...
&emsp; There are certain limitations in performing PCA, one of which is because PCA calculation using a covariance/correlation matrix is a Euclidean measure of distances. This means that it assumes the data is linearly connected, which is why PCA is a linear model of ordination. This is espcially bad in the case of ecological community analysis, where data is very sparse with a high number of 0 counts. The result of this limitation is seen in my analysis, where a "horseshoe"-like shape is visualized and is not very interpretable (<a href="#fig2">Figure 2</a>).  
&emsp; Other ordination techniques involve using a distance matrix instead of covariance/correlation matrix for finding ordered relationships. Metric multidimensional scaling (MDS), also known as principal coordinate analysis (PCoA), is one of such techniques (McCarthy, n.d; MS, n.d.; OM, n.d). MDS is exactly the same as PCA, in the sense that it performs Eigenanalysis. In fact, if you used a Euclidean measure for a distance matrix and used this to calculate MDS, the output would be the exact same as PCA (Figures <a href="#fig2">2</a> & <a href="#fig7">7</a>). MDS is an improvement compared to PCA because it allows you to provide distance measures other than Euclidean, this escaping the linear limitation of Euclidean distance; however, MDS attempts to maximize linear distances between points. So the major limitation to MDS is finding a distance measure which best fits the data.  
&emsp; Similarly, nonmetric multidimensional scaling (NMDS) also intakes a distance matrix for calculation (McCarthy, n.d; MS, n.d.; OM, n.d). This technique differs from PCA and MDS, in the sense that the order of the axes is arbritary so that no one axis is more important than the other. Although I do not have a complete grasp of this method, I understand that the main goal is to separate similar points from dissimilar points. Also, NMDS does not by maximize linear distances but by maximizes ranking orders. This process is said to be iterative, which you first choose the number of dimensions to transform data observations. The iteration process causes convergence at some point, or stops when a low stress value is calculated (a stress value is a goodness-of-fit measure). Due to time limitation on this project, I plan on looking into the NMDS algorithm in the future.
...
&emsp; For NMDS analysis, I again used the "vegan" package with the function `metaMDS()` which iteratively tries to calculate the lowest stress value (a goodness of fit measure). The number of iterations, as well as stress value, will differ per run because `metaMDS()` starts calculations at different points. For this analysis I kept the distance measure as Jaccard to compare with the MDS method. I also kept the same argument, "k=2", to calculate for components NMDS1 and NMDS2. The output graph more distinctly groups CDI cured patients with their matched controls, as compared to MDS (<a href="#fig9">Figure 9</a>).   
