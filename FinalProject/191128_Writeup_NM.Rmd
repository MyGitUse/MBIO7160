...
<u>**Data Wrangling**</u>
```{r, eval=FALSE}
#NOTE - I realized I should have done this all in a Rproject container but I didn't (laziness), which is why I have to specify all my directory paths
# I also realize I could have just downloaded FASTQ files corresponding to the 16 Cases+replicates & controls to save myself from all this trouble but it was already too late when I started & re-read the article

#Set directory for loading in files
pathOTU <- "C:/Users/Nelson/Desktop/MegaSync/Other/191114_Statsproj/StatsProjrun/OTU.txt"
pathSamples <- "C:/Users/Nelson/Desktop/MegaSync/Other/191114_Statsproj/StatsProjrun/ProjectMapping.txt"
...
#Load in metadata
  sra_result <- read_excel("C:/Users/Nelson/Desktop/MegaSync/Other/191114_Statsproj/sra_result.xlsx") #load sorted metadata
...
#HAVE TO EDIT THE pca() function FROM PCA TOOLS
  ...
  #changed !all(colnames(mat) == rownames(metadata) to all(colnames(mat) == rownames(metadata)
...
OTU <- read.table(pathOTU, header=TRUE, row.names = 1, sep="\t") 
  ...
  sra_result <- read_excel("C:/Users/Nelson/Desktop/MegaSync/Other/191114_Statsproj/sra_result.xlsx") 
...
> **Figure 1.** Screeplot PCAtools

```{r, echo=F, warning=F, message=F}
fviz_eig(ppx) # screeplot looks about the same
```

> **Figure 2.** Screeplot factoextra
...
> **Figure 3.** Biplot of PC2 vs PC1 PCAtools
...
> **Figure 4.** Biplot PC2 vs PC1 factoextra

```{r, echo=F, warning=F, message=F}
pairsplot(PCA, colby='groupings')
```

> **Figure 5.** Pairsplot PCAtools
...
> **Figure 6.** MDS plot using Euclidean distance (vegan::vegdist & stats::cmdscale)
...
> **Figure 8.** NMDS plot using Jaccard distance (vegan::metaMDS)
